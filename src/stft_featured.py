...
def spec_augment(
    ...
    Args:
        spec:             (freq_bins, time_steps) log-mag spectrogram
    ...
    for _ in range(num_freq_masks):
        f0 = np.random.randint(0, freq_mask_param + 1)
        f_start = np.random.randint(0, max(1, f - f0))
        spec[f_start : f_start + f0, :] = 0

    # Time masks
    for _ in range(num_time_masks):
        t0 = np.random.randint(0, time_mask_param + 1)
...
class BaselineSTFTFeaturedModel(nn.Module):
    ...
    def __init__(self, n_classes=4, n_fft=256, hop_length=128, dropout_rate=0.4):
        ...
        self.handcrafted_bn = nn.BatchNorm1d(self.num_handcrafted_features)
    ...
    def _extract_simplified_hrv_features(self, signals):
        ...
        for signal in signals:
            ...
            if search_start_idx >= len(autocorr):
                ...
                positive_lags_autocorr = autocorr[search_start_idx:]
                if len(positive_lags_autocorr) == 0:
                    ac_lag = 0.0
                    ac_value = 0.0
                else:
                    peak_relative_idx = np.argmax(positive_lags_autocorr)
                    peak_absolute_idx = search_start_idx + peak_relative_idx
                    ac_lag = float(peak_absolute_idx - center_idx)
                    ac_value = float(autocorr[peak_absolute_idx])

            features_list.append(
                torch.tensor([skew, kurt, iqr, ac_lag, ac_value], dtype=torch.float32, device=signal.device)
            )
    ...
    def forward(self, x, lengths):
        ...
        # STFT -> log‑mag spectrogram -> SpecAugment if training
        spectrograms = []
        for signal in x:
